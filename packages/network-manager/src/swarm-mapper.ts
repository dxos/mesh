...
export class SwarmMapper {
  ...
  private readonly _connectionSubscriptions = new ComplexMap<PublicKey, () => void>(x => x.toHex());
  ...
  constructor (
    ...
    this._subscriptions.push(_swarm.connectionAdded.on(connection => {
      this._update()
      this._connectionSubscriptions.set(connection.remoteId, connection.stateChanged.on(state => {
        this._update()
      }))
    }))
